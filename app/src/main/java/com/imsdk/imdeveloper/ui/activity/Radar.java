...
import com.imsdk.imdeveloper.ui.CustomViewPager;
 ...
 * 演示周边雷达的业务场景使用
 */
public class Radar extends Activity implements RadarUploadInfoCallback,
    ...
    public void uploadOnceClick(View v) {
        if (pt == null) {
            Toast.makeText(Radar.this, "未获取到位置", Toast.LENGTH_LONG).show();
            return;
        }
    ...
    public void searchNearby(View v) {
        if (pt == null) {
            Toast.makeText(Radar.this, "未获取到位置", Toast.LENGTH_LONG)
                    .show();
            return;
        }
        pageIndex = 0;
    ...
    public void onGetNearbyInfoList(RadarNearbyResult result,
        ...
        if (error == RadarSearchError.RADAR_NO_ERROR) {
            Toast.makeText(Radar.this, "查询周边成功", Toast.LENGTH_LONG)
                    .show();
            // 获取成功
            listResult = result;
            ...
            curPage = 0;
            totalPage = 0;
            Toast.makeText(Radar.this, "查询周边失败", Toast.LENGTH_LONG)
                    .show();
        }
    ...
    public void onGetUploadState(RadarSearchError error) {
        ...
        if (error == RadarSearchError.RADAR_NO_ERROR) {
            // 上传成功
            if (!uploadAuto) {
                Toast.makeText(Radar.this, "单次上传位置成功", Toast.LENGTH_LONG)
                        .show();
            }
        } else {
            // 上传失败
            if (!uploadAuto) {
                Toast.makeText(Radar.this, "单次上传位置失败", Toast.LENGTH_LONG)
                        .show();
            }
        }
    }
    ...
    public void onGetClearInfoState(RadarSearchError error) {
        ...
        if (error == RadarSearchError.RADAR_NO_ERROR) {
            // 清除成功
            Toast.makeText(Radar.this, "清除位置成功", Toast.LENGTH_LONG)
                    .show();
        } else {
            // 清除失败
            Toast.makeText(Radar.this, "清除位置失败", Toast.LENGTH_LONG)
                    .show();
        }
    }
    ...
    public boolean onMarkerClick(Marker marker) {
        ...
        mBaiduMap.hideInfoWindow();
        if (marker != null) {
            popupText = new TextView(Radar.this);
    ...
    private class RadarResultListAdapter extends BaseAdapter {
        ...
        public View getView(int index, View convertView, ViewGroup parent) {
            convertView = View.inflate(Radar.this,
